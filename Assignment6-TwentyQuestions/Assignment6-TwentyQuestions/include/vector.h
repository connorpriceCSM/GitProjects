 ...
 * --------------------
 * Aug 10 2015 by cpw - removed all uses of foreach
 *                    - added initializer list constructor
 *                    - added range constructor
 * Aug 11 2015 by cpw - fix for range constructor to prevent compiler confusion
 */
...
public:
/*
 * Range and initializer list support
 * ----------------------------------
 * The range constructor makes it possible to construct a vector easily from
 * any collection supporting iterators.  The remaining constructor and 
 * operator= provide support for C++ 11 initializer lists.
 */
// The range constructor has some extra template juju to ensure that the
// compiler can distinguish this constructor taking 2 arguments from the
// other constructor taking 2 arguments when the 2 arguments are both ints.
   template <typename _InputIterator, 
             typename = typename std::enable_if<
                !std::is_integral<_InputIterator>::value>::type> 
...
Vector<ValueType>::Vector(_InputIterator first, _InputIterator last) {
   count = capacity = 0;
   elements = NULL;
...
template <typename ValueType>
Vector<ValueType>::Vector(std::initializer_list<ValueType> lst) {
   count = capacity = lst.size();
