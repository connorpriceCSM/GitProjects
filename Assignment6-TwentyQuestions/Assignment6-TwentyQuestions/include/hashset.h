 ...
 * -------------------------------------
 * Returns the first value in the set in the order established by the keys.
 ...
 */
   ...
   HashSet(std::initializer_list<ValueType> lst);
...
template <typename ValueType>
template <typename InputIterator>
HashSet<ValueType>::HashSet(InputIterator first, InputIterator last) {
...
template <typename ValueType>
HashSet<ValueType>::HashSet(std::initializer_list<ValueType> lst) {
   for (auto e: lst) {
      this->add(e);
   }
}
