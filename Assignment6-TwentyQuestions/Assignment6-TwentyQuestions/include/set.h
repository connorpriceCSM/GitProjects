 ...
 * key.  If the set is empty, <code>first</code> generates an error.
 */
 ...
 * ----------------------------------
 * The range constructor makes it possible to construct a set easily from
...
template <typename ValueType>
template <typename InputIterator>
Set<ValueType>::Set(InputIterator first, InputIterator last) {
...
template <typename ValueType>
Set<ValueType>::Set(std::initializer_list<ValueType> lst) {
   for (auto e: lst) {
      this->add(e);
   }
}
