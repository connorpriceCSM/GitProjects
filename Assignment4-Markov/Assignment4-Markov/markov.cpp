...
markov::markov() {
	_model_map.put("brute", new brute_model);
	_model_map.put("map", new map_model);
    _model_map.put("word", new word_model);
}

// ~markov()
// Releases any models created in constructor.
markov::~markov() {
	for (string key: _model_map) {
		delete _model_map[key];
	}
}
...
bool markov::_set_model(string model_type) {
	if (model_type == "") {
		...
		cout << endl;
		return true;
	} else if (_model_map.containsKey(model_type)) {
		_initialized = false;
